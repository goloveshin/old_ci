...
inline size_t filesize(const filename_t& filename)
{
    ...
    throw spdlog_ex("Failed opening file for stat command", errno);
}

// Change access mode of the file
// Example: rw-rwxr-- -> mode = 674
inline bool file_chmod(const filename_t& filename, const mode_t& mode)
{
    ...
    {
        throw spdlog_ex("stat failed on file " + filename, errno);
    }

    if (mode == file_info.st_mode)
    {
        return true;
    }
    ...
    {
//        throw spdlog_ex("chmod failed for file " + filename, errno);
    }
