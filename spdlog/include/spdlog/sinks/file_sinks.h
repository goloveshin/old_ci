...
public:
    rotating_file_sink(const filename_t &base_filename,
                       ...
                       mode_t chmod = S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) :
    ...
    filename_t get_file_name() const { return _base_filename; }
    std::size_t get_max_size() const { return _max_size; }
    std::size_t get_max_files() const { return _max_files; }
...
private:
    ...
    void _rotate()
    {
        ...
        for (auto i = _max_files; i > 0; --i)
        {
            ...
            {
//                ALOGD("TaigaRFC: going to remove file: %s", target.c_str());
